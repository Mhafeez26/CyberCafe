...
public class BillingPanel extends JPanel{
    ...
    public BillingPanel(){
        ...
        String[] cols ={"Session ID","PC","Customer","Start Time","Hrs","Rate/hr","Subtotal","Disc%","Tax%","Total"};
        ...
        btnLoad.setPreferredSize(new Dimension(160,32));
        btnEnd.setPreferredSize(new Dimension(150,32));
        btnRow.add(btnLoad); btnRow.add(btnEnd); btnRow.add(btnPrint);
    ...
    private void loadActiveSessions(){
        ...
        try{
            ...
            while (rs.next()){
                ...
                double[] bill = backend.calculateBill(rate,start.getTime(),disc,tax);
                tableModel.addRow(new Object[]{
                    id,pc,cust,start.toString().substring(0,16),
                    String.format("%.2f",bill[0]),rate,
                    String.format("%.2f",bill[1]),
                    disc,tax,
                    String.format("%.2f",bill[4])
               });
           }
    ...
    private void endSession(){
        ...
        double total  = Double.parseDouble((String) tableModel.getValueAt(row,9));

        if (backend.endSession(sessionId,total)){
            generateReceipt(row,total);
            JOptionPane.showMessageDialog(this,"Session End. Total: PKR " + String.format("%.2f",total));
            loadActiveSessions();
       }
   }
    ...
    private void printReceipt(){
        if (receiptArea.getText().isEmpty()){
            ExceptionHandler.handleValidationError("To print first then session end",this);
            return;
       }
        try{
            receiptArea.print();
       } catch (Exception e){ ExceptionHandler.handleGeneralException(e,this);}
   }
}
